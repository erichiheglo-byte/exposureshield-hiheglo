...
const EXPOSURE_SHIELD = {
  ...
  init: function () {
    ...
    // Set up mobile menu (safe no-op if elements do not exist)
    this.setupMobileMenu();

    // Set up contact form (updated to match your HTML IDs)
  ...
  setupScanButton: function () {
    // Your HTML uses: <button id="checkBtn">
    const button = document.getElementById("checkBtn");
    if (!button) {
      console.error("❌ Scan button (#checkBtn) not found");
      return;
    }
    ...
    // Your HTML uses: <input id="emailInput">
  ...
  // Set up mobile menu toggle (safe no-op if not present)
  ...
  // Set up contact form (matches your HTML: sendMessageBtn + contactName/contactEmail/contactMessageInput)
  ...
  async handleScan(button) {
    ...
    try {
      // Call API (POST first to match your HTML; GET fallback supported)
      const data = await this.checkEmail(email);

      // Prefer your existing modal renderer (from your HTML script) if present
      if (typeof window.showResultsModal === "function") {
        window.showResultsModal(data, email);
      } else {
        // Fallback: render into results container if you use that pattern
        const breaches = Array.isArray(data) ? data : (data?.breaches || []);
  ...
  async checkEmail(email) {
    // POST (matches your HTML implementation)
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) return { safe: true, breaches: [], pastes: [], exposedCount: 0 };
        throw new Error(data?.error || `API error: ${response.status}`);
      }

      return data;
    } catch (postErr) {
      // GET fallback (keeps compatibility if your backend expects querystring)
      const url = `${this.config.API_ENDPOINT}?email=${encodeURIComponent(email)}`;
      console.log(`🌐 POST failed, trying GET: ${url}`);
  ...
  // Show results (fallback renderer)
  ...
  getBreachesHTML: function (email, breaches) {
    ...
    // FIX: safe numeric sum (prevents NaN + supports strings)
